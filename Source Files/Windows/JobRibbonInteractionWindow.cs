...
public sealed class JobRibbonInteractionWindow : Window, IDisposable
{
    private readonly Plugin plugin;
    private float scale;

    public JobRibbonInteractionWindow(Plugin plugin)
        : base("RE:Frame Job Ribbon Input###REFrameJobRibbonInput",
    ...
    public override void Draw()
    {
        ...
        var clicked = ImGui.InvisibleButton("##job-ribbon-toggle", size);
        ...
        if (hovered)
        {
            ...
            draw.AddRectFilled(
                ...
                ImGui.ColorConvertFloat4ToU32(new Vector4(
                    ...
                    theme.Accent.Y,
                    theme.Accent.Z,
                    plugin.IsJobSwitcherOpen ? 0.31f : 0.20f)),
                5f * scale);
            draw.AddRect(
                min,
                max,
            ...
            ImGui.SetTooltip(plugin.IsJobSwitcherOpen ? "Close Job Deck" : "Open Job Deck");
        }

        if (clicked)
            plugin.ToggleJobSwitcher();
    }
